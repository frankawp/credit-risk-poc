...
- 数据文件 → `outputs/data/`（辅助产物）
- **核心产出是业务报告，数据文件仅作为计算支撑**
...
**输出**：`outputs/reports/01_data_overview.md`
- 表结构概览（表名、字段数、记录数）
- 主键/外键识别结果
- 表间关系推断
- 数据质量摘要（缺失率、异常值）
...
**输出**：`outputs/reports/01_data_overview.md`（补充业务理解）
- 业务含义确认
- 目标变量定义
- 变量机会点分析
...
**输出**：`outputs/reports/02_feature_design.md`
- 变量假设清单（名称、逻辑、假设、优先级）
- 变量计算口径（公式、数据来源、时间窗口）
- 业务含义说明（预期方向、风险解读）
...
**代码模板**：
```python
# 文件: outputs/data/src/{theme}_features.py
import pandas as pd

def build_{theme}_features(frames: dict[str, pd.DataFrame], anchor: pd.DataFrame) -> pd.DataFrame:
    """
    {业务假设说明}
...
**输出**：
- `outputs/reports/02_feature_design.md`（补充实现细节）
- `outputs/data/feature_matrix.parquet`（辅助数据文件）
...
**输出**：
- `outputs/reports/03_feature_evaluation.md`（效果评估报告）
- `outputs/reports/04_stability_check.md`（稳定性检查报告）
- `outputs/reports/05_selection_summary.md`（筛选总结报告）
- `outputs/reports/mining_report.md`（完整挖掘报告，合并上述内容）
...
**注意**：代码是辅助产物，核心产出是 `outputs/reports/02_feature_design.md` 中的变量设计报告。

---

## 标准交付模板

**核心产出是业务报告，面向业务人员呈现完整挖掘过程。**

### 报告文件

| 报告 | 内容 |
|------|------|
| `01_data_overview.md` | 数据概览：表结构、主外键、数据质量 |
| `02_feature_design.md` | 变量设计：假设清单、计算口径、业务含义 |
| `03_feature_evaluation.md` | 效果评估：单变量效果、假设验证、业务解读 |
| `04_stability_check.md` | 稳定性检查：PSI 分析、时间分片、分组一致性 |
| `05_selection_summary.md` | 筛选总结：入选变量、筛选方法、最终效果 |
| `mining_report.md` | 完整报告：合并上述内容，供业务人员阅读 |

### 每份报告的必备要素

1. **结论摘要**：这轮做了什么、最重要发现是什么
2. **关键指标**：AUC、IV、PSI 等核心指标表格
3. **业务解释**：变量含义、筛选逻辑、效果解读
4. **假设验证**：哪些假设成立、哪些不成立
5. **下一步建议**：下一轮该做什么
...
- **核心产出是报告**：业务报告优先于数据文件，面向业务人员
- **报告格式 Markdown**：所有报告使用 Markdown 格式，便于阅读和分享
- **插件目录只读**：禁止修改插件目录下的任何文件
- **报告写在 reports 目录**：`outputs/reports/`
- **数据文件写在 data 目录**：`outputs/data/`（辅助产物）
