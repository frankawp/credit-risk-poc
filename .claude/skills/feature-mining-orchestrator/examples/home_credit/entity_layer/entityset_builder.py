...
def _load_sampled_frames(config: AutoFeatureConfig, raw_dir: Path) -> Dict[str, pd.DataFrame]:
    app_cols = [
        "SK_ID_CURR",
        "TARGET",
        "AMT_INCOME_TOTAL",
...
def build_entityset_for_auto(
    ...
    参数:
        config: 自动特征配置
        paths: 路径配置，如果为 None 则使用默认路径
    """
    if paths is None:
        paths = EnginePaths()
    raw_dir = paths.raw_dir

    frames = _load_sampled_frames(config, raw_dir)
