...
| 字段 | 来源表 | 必需/可选 |
|------|--------|----------|
| DAYS_DECISION | previous_application | 必需 |
| SK_ID_CURR | previous_application | 必需 |
...
| 字段 | 来源表 | 必需/可选 |
|------|--------|----------|
| DAYS_CREDIT | bureau | 必需 |
| CREDIT_ACTIVE | bureau | 可选 |
...
| 变量名 | 计算逻辑 | 业务假设 |
|--------|----------|----------|
| `velocity_prev_channel_count` | 历史申请渠道去重数 | 多渠道申请可能是多头 |
| `velocity_prev_product_types` | 历史申请产品类型数 | 多产品申请可能资金需求大 |

**数据需求**：
| 字段 | 来源表 | 必需/可选 |
|------|--------|----------|
| NAME_PORTFOLIO | previous_application | 可选 |
| NAME_PRODUCT_TYPE | previous_application | 可选 |
...
3. **低优先级**（数据依赖）：
   - velocity_bureau_active_count
   - velocity_prev_channel_count

## 扩展方向

- 更短时间窗口（1天、3天）
- bureau_balance 月度轨迹分析
- 申请时间分布熵值
- 跨平台多头检测
